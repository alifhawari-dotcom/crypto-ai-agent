...
async def send_to_telegram(text):
    ...
    # BAGIAN INI SUDAH SAYA RAPIKAN AGAR TIDAK TERPOTONG SAAT DI-COPY
    payload = {
        "chat_id": TG_CHAT_ID, 
        "text": f"🎯 **TACTICAL HUD: TOP 3 OPTIONS**\n\n{text}", 
        "parse_mode": "Markdown"
    }
...
async def main():
    ...
    logging.info("Memindai dan meranking koin (Candle 1H)...")
    try:
        data = await get_high_precision_data()
        logging.info(f"Berhasil meranking {len(data)} koin teratas.")
        
        analysis = await ask_ai_agent(data) 
        await send_to_telegram(analysis)
        logging.info("Laporan 3 Opsi Trading sukses dikirim ke Telegram!")
    except Exception as e:

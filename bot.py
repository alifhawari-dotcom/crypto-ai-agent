...
# ============================================================
# CONSTANTS (V4.5 — APEX FINAL)
# ============================================================

# ── Gemini Model Switcher (Anti Error 429) ───────────────────
# Urutan prioritas: model terbaru + cepat dulu, fallback ke versi lama
GEMINI_MODELS = [
...
RANKED_CANDIDATES   = 10   # Turun dari 12 → hemat 2 request fase 2
CANDLES_REQUIRED    = 100

# ── Risk Management ──────────────────────────────────────────
FIXED_RISK_USD = 1.50   # Risiko per trade (USD) — ubah sesuai modal
RR_TP1         = 2.0    # R:R TP1 (Pine Script: 1:2)
RR_TP2         = 3.5    # R:R TP2 (extended: 1:3.5)
SL_ATR_BUFFER  = 0.5    # Buffer SL di balik swing — anti liquidity sweep
QTY_MAX_CAP    = 99999  # Safety cap qty untuk altcoin harga sangat kecil

# ── Timeframes ───────────────────────────────────────────────
TF_MACRO  = '1h'   # Trend anchor
TF_STRUCT = '15m'  # Swing level + entry trigger
TF_MICRO  = '5m'   # Whale/volume anomaly
...
def calc_swing_levels(df: pd.DataFrame, lookback: int = SWING_LOOKBACK) -> dict:
    ...
    for lb in range(lookback, 2, -1):
        ...
        if sh and sl:
            break
    # Fallback ke rolling max/min jika swing formal tidak ditemukan
    if not sh:
...
def finalize_candidate(c: dict) -> dict:
    ...
    sell_stop = round(sl * 0.999, 6) if sl < price else round(price - atr * 0.2, 6)

    sl_long  = round(sl  - atr * SL_ATR_BUFFER, 6)  # Di bawah support — anti sweep
    sl_short = round(sh  + atr * SL_ATR_BUFFER, 6)  # Di atas resistance — anti sweep
...
async def ask_ai_agent(data_list: list) -> str:
    ...
    # Token-slim format — padat, cukup untuk AI memilih
    rows = []
    for d in data_list:
        act  = "L" if d['Composite'] > 0 else "S"
        cncl = "⚠️CANCEL" if (act=="L" and d['Cancel_Long']) or (act=="S" and d['Cancel_Short']) else ""
        rows.append(
            f"{d['Symbol']}|{act}{cncl}|{d['Composite']}({d['Matrix_Sync']})|"
            f"1H:{d['Trend_1h']}|SM:{d['SM_Signal']}|3TF:{'✅' if d['Aligned'] else '❌'}|"
            f"Entry:{'BUY@'+str(d['Buy_Stop']) if act=='L' else 'SELL@'+str(d['Sell_Stop'])}|"
            f"SL:{''+str(d['SL_Long']) if act=='L' else str(d['SL_Short'])}|"
            f"TP1:{''+str(d['TP1_Long']) if act=='L' else str(d['TP1_Short'])}|"
            f"Qty:{''+str(d['Qty_Long']) if act=='L' else str(d['Qty_Short'])}"
        )

    prompt = (
        "Kamu AI Intraday Breakout Trader. Pilih 4 koin terbaik dari data berikut.\n"
        "ABAIKAN koin bertanda ⚠️CANCEL.\n"
        "Prioritas: SM=NUCLEAR/NUC+SQZ, 3TF=✅, Trend 1H searah aksi.\n\n"
        + "\n".join(rows) +
        f"\n\nFORMAT WAJIB (Markdown, ringkas):\n"
        "### [N]. [KOIN] — [LONG/SHORT]\n"
        "**Konfluensi:** [Max 1 kalimat — sebut SM signal + trend]\n"
        f"**Entry:** [BUY/SELL STOP di harga] | **Qty:** [jumlah] _(Risk ${FIXED_RISK_USD})_\n"
...
# ============================================================
# TELEGRAM — FORMAT RINGKAS + HARGA BISA DICOPY
# Setiap koin dikirim sebagai PESAN TERPISAH.
# Harga Entry/SL/TP dibungkus `backtick` — di Telegram mobile,
# tap backtick text → otomatis muncul tombol "Copy".
# ============================================================

def build_trade_message(d: dict, rank: int) -> str:
    """
    Format pesan per koin — ringkas, padat, semua harga bisa dicopy.
    Backtick = monospace = tap to copy di Telegram mobile & desktop.
    """
...
async def send_to_telegram(text: str, header: str = ""):
    """Kirim header + analisis AI dulu, lalu tiap ringkasan koin sebagai pesan terpisah."""
    url = f"https://api.telegram.org/bot{TG_TOKEN}/sendMessage"

    async with aiohttp.ClientSession() as session:

        async def _post(payload: dict) -> bool:
            """Post satu pesan dengan retry + fallback plain text."""
            for attempt in range(1, MAX_RETRIES + 1):
                try:
                    async with session.post(
                        url, json=payload,
                        ...
                        if resp.status == 400 and "parse_mode" in payload:
                            # Format Markdown error → kirim ulang tanpa formatting
        ...
        # Kirim analisis AI (chunked jika panjang)
        full  = header + text
        chunks = [full[i:i+4000] for i in range(0, len(full), 4000)]
        for chunk in chunks:
            await _post({"chat_id": TG_CHAT_ID, "text": chunk, "parse_mode": "Markdown"})
            if len(chunks) > 1:
                await asyncio.sleep(1)


async def send_trade_cards(data_list: list):
    """
    Kirim kartu ringkasan per koin sebagai pesan terpisah.
    Harga dalam backtick = bisa dicopy dengan tap di Telegram.
    Hanya kirim koin yang tidak kena cancel warning (atau tetap kirim tapi tandai).
    """
    url = f"https://api.telegram.org/bot{TG_TOKEN}/sendMessage"

    async with aiohttp.ClientSession() as session:
        for i, d in enumerate(data_list, 1):
            msg = build_trade_message(d, i)
            ...
            for attempt in range(1, MAX_RETRIES + 1):
                try:
                    async with session.post(
                        ...
                        if resp.status == 400 and "parse_mode" in payload:
                            plain = {k: v for k, v in payload.items() if k != "parse_mode"}
                            await session.post(url, json=plain)
                            break
                except Exception as e:
                    logging.warning(f"Trade card {d['Symbol']} attempt {attempt}: {e}")
                if attempt < MAX_RETRIES:
                    await asyncio.sleep(RETRY_DELAY)
            await asyncio.sleep(0.8)   # Jeda antar pesan agar tidak flood
...
async def main():
    ...
    try:
        ...
        if not data:
            ...
            payload = {"chat_id": TG_CHAT_ID, "text": "⚠️ Scan gagal: tidak ada data."}
            async with aiohttp.ClientSession() as s:
                await s.post(f"https://api.telegram.org/bot{TG_TOKEN}/sendMessage", json=payload)
            return

        logging.info(f"✅ {len(data)} koin. Kirim ke Gemini ({GEMINI_MODELS[0]})...")
        analysis = await ask_ai_agent(data)

        # Pesan 1: Header + analisis AI (ringkasan naratif)
        await send_to_telegram(analysis, header=header)
        logging.info("✅ Analisis AI terkirim.")

        await asyncio.sleep(1.5)

        # Pesan 2+: Kartu per koin — harga bisa dicopy langsung
        # Hanya ambil data top 5 yang dipilih (sesuai jumlah output AI)
        top5 = data[:5]
        await send_trade_cards(top5)
        logging.info(f"✅ {len(top5)} kartu trade terkirim.")
        ...
        try:
            async with aiohttp.ClientSession() as s:
                await s.post(
                    ...
                    json={"chat_id": TG_CHAT_ID, "text": f"❌ *SYSTEM ERROR*\n`{str(e)[:300]}`",
                          "parse_mode": "Markdown"}
                )
        except Exception:
            pass

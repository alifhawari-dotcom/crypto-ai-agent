...
def finalize_screener(c):
    p15 = c['power_15m']
    p1h = c.get('power_1h', c['power_15m'])
    p5m = c.get('power_5m', c['power_15m'])
    ...
    sm_bonus  = (23 if 'NUC+SQZ' in sm_lvl else
                 15 if is_nuclear else
                  7 if 'ACTIVE' in sm_lvl else 0)
    if sm_bonus > 0:
        comp += sm_bonus if comp > 0 else -sm_bonus

    aligned = ((p5m > 0 and p15 > 0 and p1h > 0) or
               (p5m < 0 and p15 < 0 and p1h < 0))
    if aligned:
    ...
    c['Conviction'] = conv
    c['Checklist']  = (int(has_fvg) + int(aligned) +
                       int(not is_side) + int(conv >= CONV_VALID) + int(has_ob))

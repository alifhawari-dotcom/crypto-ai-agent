...
def compute_regime_score(pct_up_1h, pct_up_4h=None):
    """Skor regime [-1, +1] dari breadth (% koin naik) DUA HORIZON: 1h dan 4h,
    keduanya dari sampel top-volume Gate.io (lihat _breadth_shortterm()).

    Sengaja bukan breadth 24-jam: itu ditemukan kontradiktif dengan kuadran
    OI (yang basisnya 15m/1h/4h) -- breadth 24h bisa "bullish" karena
    kenaikan semalam, padahal momentum beberapa jam terakhir sudah berbalik
    turun. 1h & 4h sebanding langsung secara horizon waktu dengan sinyal
    kuadran, jadi tidak lagi membandingkan dua skala waktu berbeda.

    +1.0 = seluruhnya naik & kedua horizon sepakat penuh
    -1.0 = seluruhnya turun & kedua horizon sepakat penuh
     0.0 = 50/50 (choppy) ATAU 1h dan 4h saling bertentangan arah

    Kalau breadth 4h gagal dihitung (pct_up_4h=None), fallback ke breadth 1h
    saja tapi skor didiskon 30% -- konfirmasi horizon lebih panjang tidak
    tersedia, jadi confidence diturunkan alih-alih dianggap sama kuatnya.
    Return (skor, has_cross_confirm).
    """
    score_1h = (pct_up_1h - 50.0) / 50.0  # -1..+1

    if pct_up_4h is None:
        return round(score_1h * 0.7, 3), False

    score_4h = (pct_up_4h - 50.0) / 50.0
    agree = (score_1h > 0) == (score_4h > 0) or (abs(score_1h) < 0.05 and abs(score_4h) < 0.05)

    if not agree:
        # 1h dan 4h bertentangan arah -> SINYAL "tidak jelas", ditarik ke nol
        # (bukan dirata-ratakan naif, yang akan menyembunyikan konfliknya).
        return 0.0, False

    return round((score_1h + score_4h) / 2, 3), True
...
def build_regime_section(pct_up_1h, pct_up_4h, weekend, n_1h=None, n_4h=None):
    """Return string siap ditempel ke pesan Telegram YANG SAMA, di ATAS
    daftar kuadran koin. Dipanggil dari build()."""
    score, has_confirm = compute_regime_score(pct_up_1h, pct_up_4h)
    ...
    sample_note = f" (sampel {n_1h}" + (f"/{n_4h} koin" if n_4h else " koin") + ")" if n_1h else ""

    lines = [
        f"{emoji} <b>REGIME: {label}</b>{confirm_note}",
        f"Skor: {score:+.2f} (1h {pct_up_1h:.0f}% naik"
        + (f", 4h {pct_up_4h:.0f}% naik" if pct_up_4h is not None else "")
        + f"){sample_note}",
        f"<i>{advice}</i>",
    ]
    if reversal_msg:
...
BREADTH_SAMPLE = 40  # jumlah koin top-volume dipakai hitung breadth jangka-pendek
                      # (sampel, bukan seluruh universe -- pola sama seperti
                      # VOL_SAMPLE di _vol_baseline, supaya tidak membebani API)


async def _breadth_shortterm(s, uni, iv, sem):
    """Breadth (% naik) dari candle TERTUTUP TERAKHIR pada timeframe `iv`,
    untuk BREADTH_SAMPLE koin top-volume. Reuse px_change() yang sudah ada
    di kuadran OI -- horizon waktu sama, jadi sebanding secara langsung
    dengan sinyal kuadran, tidak seperti change_percentage 24 jam bawaan
    Gate.io yang basisnya beda skala waktu.

    Pakai semaphore `sem` yang SAMA dengan screen() -- supaya request
    breadth ini tidak menambah beban paralel di luar batas SEM_N yang
    sudah dikalibrasi untuk menghindari rate limit Gate.io.

    Return (pct_up, n_sampled) atau (None, 0) kalau data tidak cukup.
    """
    sample = uni[:BREADTH_SAMPLE]

    async def _one(it):
        async with sem:
            return await px_change(s, it['c'], iv)

    changes = await asyncio.gather(*[_one(it) for it in sample], return_exceptions=True)
    valid = [c for c in changes if isinstance(c, (int, float))]
    if len(valid) < BREADTH_SAMPLE // 2:  # kurang dari separuh sampel valid -> jangan dipercaya
        return None, len(valid)
    up = sum(1 for c in valid if c > 0)
    return (up / len(valid) * 100.0), len(valid)


async def market_context(s, uni, sem):
    """Rangkuman kondisi pasar. Regime section sekarang pakai breadth 1h & 4h
    (horizon sama dengan kuadran OI) sebagai basis utama, BUKAN breadth 24-jam
    bawaan Gate.io -- itu ditemukan kontradiktif dengan kuadran OI saat
    breadth 24h bullish tapi mayoritas kandidat lolos justru condong turun
    (observasi live 25 Sep 2026). Breadth 24-jam (Gate & Hyperliquid) tetap
    ditampilkan di baris deskriptif sebagai konteks makro, bukan basis skor."""
    ...
    n = up + down
    pct_up_24h = (up / n * 100.0) if n > 0 else None  # baris deskriptif SAJA, bukan basis skor
    ...
    if hl:
        hl_n = hl['up'] + hl['down']
        if hl_n > 0:
            hl_pct_up_24h = hl['up'] / hl_n * 100.0

    # Breadth jangka-pendek -- basis UTAMA regime, horizon sama dengan kuadran OI
    pct_up_1h, n_1h = await _breadth_shortterm(s, uni, '1h', sem)
    pct_up_4h, n_4h = await _breadth_shortterm(s, uni, '4h', sem)
    ...
    # --- Regime alignment section: basis breadth 1h/4h, BUKAN 24 jam ---
    if pct_up_1h is None:
        regime_section = "⚪ <b>REGIME: DATA TIDAK CUKUP</b>\n<i>Breadth 1h/4h gagal dihitung siklus ini -- lewati bagian regime.</i>"
    else:
        regime_section = build_regime_section(pct_up_1h, pct_up_4h, weekend, n_1h, n_4h)

    # --- Deskriptif tambahan: breadth 24-jam sebagai KONTEKS MAKRO, label
    #     eksplisit "24H" supaya tidak tertukar dengan skor regime di atas ---
    ctx_label = "24H"
    if pct_up_24h is not None:
        if pct_up_24h >= 60:   regime, emo = "BULLISH", "🟢"
        elif pct_up_24h <= 40: regime, emo = "BEARISH", "🔴"
        else:                  regime, emo = "NEUTRAL", "⚪"
        lines = [f"{emo} <b>{ctx_label} {regime}</b> · {pct_up_24h:.0f}% naik ({up}↑/{down}↓)"]
    else:
        lines = [f"⚪ <b>{ctx_label}</b> · data tidak cukup"]
    ...
    if hl:
        ...
        lines.append(f"Hyperliquid (24h): vol {money(hl['vol'])} · OI {money(hl['oi'])} "
                     f"· {hl_pct:.0f}% naik")
...
async def main():
    async with aiohttp.ClientSession(headers={'Accept': 'application/json'}) as s:
        ...
        regime_section, ctx = await market_context(s, uni, sem)
